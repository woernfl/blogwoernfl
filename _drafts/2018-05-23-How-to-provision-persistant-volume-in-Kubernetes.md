...
- `Automatically creating the persistant volume` by letting Kubernetes talk with your storage backend so that it can provision itself what it needs
...
Tested on Minikube, if you are not running Minikube, there will some minoir change to do, mainly in the `persistant volume` description.
...
spec:
  ...
  hostPath:
    path: /data/mysql-pv/
```
3.Deploy a `persistant volume claim`:
```yaml
apiVersion: v1
    ...
    spec:
      ...
      - name: mysql-persistent-storage
        persistentVolumeClaim:
          claimName: mysql-pvc
```
5.Expose internaly to the cluster and via DNS the MySQL container via a `service`:
```yaml
apiVersion: v1
kind: Service
metadata:
...
Additionally, once the many `storage class` have been created by `John`, `Bob` can create whithout delay the `persistant volume` he need.
